...
class CP extends ChangeNotifier {
  ...
  // TODO: DELETE - Replaced by SiteSyncHandler for shared site handling
  // Future<bool> storeSharedSite(String UID, String siteID) async { ... }
  // Future<bool> downloadSite(String ownerUID, String siteID) async { ... }

  // TODO: DELETE - Shared sites will use SiteSyncHandler.downloadSharedSites()
  ...
  // REMOVED: addSite(), updateSite(), updateSiteLocalVariables()
  // Use SiteDao + SiteSyncHandler instead (offline-first architecture)

  // TODO: DELETE - Replaced by SiteDao.watchAllSites() + SiteSyncHandler.downloadAllOwnedSites()
  ...
  loadProfileOfUser() async {
    ...
    if (firebaseUser.emailVerified) {
      ...
      try {
        ...
        // Sites now loaded via SiteDao (local) + SiteSyncHandler (download)
        // MainMenu._checkForPendingSync() triggers download when needed
        if (kDebugMode) print('MAIN_LP: Site loading handled by MainMenu + SiteSyncHandler');
        if (profileResult) {
