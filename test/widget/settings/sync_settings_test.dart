...
import '../profile/manual_mocks.dart';

// Sync Settings Screen implementation (TDD - implementation will follow)
...
void main() {
  ...
  group('Sync Settings Screen Tests', () {
    ...
    setUp(() async {
      ...
      SharedPreferences.setMockInitialValues({
        ...
        'last_sync_time': DateTime.now().subtract(const Duration(hours: 2)).millisecondsSinceEpoch,
        'total_data_synced': 1024 * 1024 * 5, // 5 MB
        'sync_error_count': 2,
      });
    ...
    group('Settings UI Tests', () {
      testWidgets('displays auto-sync toggle with correct state', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SyncSettingsScreen(syncServiceOverride: mockSyncService),
          ),
        );
        ...
        expect(find.text('Automatically sync when online'), findsOneWidget);
      ...
      testWidgets('toggles auto-sync and persists state', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SyncSettingsScreen(syncServiceOverride: mockSyncService),
          ),
      ...
      testWidgets('WiFi-only toggle disabled when auto-sync off', (tester) async {
        SharedPreferences.setMockInitialValues({
        ...
        expect(wifiSwitch.onChanged, isNull);
      });
      
      testWidgets('shows sync interval selector', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SyncSettingsScreen(syncServiceOverride: mockSyncService),
          ),
      ...
      testWidgets('opens interval selection dialog', (tester) async {
        SharedPreferences.setMockInitialValues({
        ...
        expect(find.text('Select Sync Interval'), findsOneWidget);
    ...
    group('Management Options Tests', () {
      testWidgets('shows clear sync queue confirmation', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SyncSettingsScreen(syncServiceOverride: mockSyncService),
          ),
        ...
        await tester.tap(find.text('Clear sync queue'));
        await tester.pumpAndSettle();
        
        expect(find.text('Clear Sync Queue?'), findsOneWidget);
        expect(find.text('This will remove all pending sync items. This action cannot be undone.'), findsOneWidget);
      });
      
      testWidgets('triggers force sync now', (tester) async {
        ...
        await tester.tap(find.text('Force sync now'));
        await tester.pump();
      ...
      testWidgets('shows reset sync state warning', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SyncSettingsScreen(syncServiceOverride: mockSyncService),
          ),
        ...
        expect(find.text('This will reset all sync flags and require a full re-sync. Continue?'), findsOneWidget);
      });
      
      testWidgets('opens sync history dialog', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SyncSettingsScreen(syncServiceOverride: mockSyncService),
          ),
    ...
    group('Statistics Display Tests', () {
      ...
      testWidgets('displays data synced with correct formatting', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SyncSettingsScreen(syncServiceOverride: mockSyncService),
          ),
      ...
      testWidgets('shows error count when errors present', (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: SyncSettingsScreen(syncServiceOverride: mockSyncService),
          ),
        ...
        expect(find.text('Error count'), findsOneWidget);
        expect(find.text('2 errors in last 24 hours'), findsOneWidget);
      });
      
      testWidgets('displays sync frequency bar chart', (tester) async {
        // Set up daily sync counts
        final prefs = {
          'auto_sync_enabled': true,
        };
        
        // Add sync counts for last 7 days
        for (int i = 0; i < 7; i++) {
          final date = DateTime.now().subtract(Duration(days: i));
          final key = '${date.year}-${date.month}-${date.day}';
          prefs['sync_count_$key'] = i * 2; // Varying counts
        }
        ...
        expect(find.text('Sync Frequency (Last 7 days)'), findsOneWidget);
